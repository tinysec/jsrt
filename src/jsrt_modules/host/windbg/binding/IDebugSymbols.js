...
{
	var info = process.reserved.hostDependBindings.windbg_IDebugSymbols_GetNameByOffset( Number64(arg_address) );
	if ( !info )
	{
		return;
	}
...
function GetOffsetByName( arg_name )
{
	...
	return Number64( process.reserved.hostDependBindings.windbg_IDebugSymbols_GetOffsetByName( arg_name ) );
}
...
function GetModuleByModuleName( arg_name ,arg_startIndex )
{
	...
	var info = process.reserved.hostDependBindings.windbg_IDebugSymbols_GetModuleByModuleName( arg_name , arg_startIndex );
...
function GetFieldOffset( arg_typename , arg_fieldname )
{
	...
	return Number64( process.reserved.hostDependBindings.windbg_IDebugSymbols_GetFieldOffset( info.imageBase , typename , arg_fieldname ) );
}
